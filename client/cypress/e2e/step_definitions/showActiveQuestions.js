...
function createQuestion(title, text, tag, username) {
  ...
  // Intercept the addQuestion API call and wait for it to complete
  cy.intercept("POST", "/question/addQuestion").as("addQuestion");
  cy.contains("Post Question").click();

  // Wait for the addQuestion API call to complete
  cy.wait("@addQuestion");

  // Verify the question appears on the page before proceeding
...
Then(
  ...
  () => {
    const newActiveOrder = [Q4_TITLE, Q5_TITLE, Q1_TITLE, Q2_TITLE, Q3_TITLE];
    cy.get(".postTitle").each(($el, index, $list) => {
