...
const populate = async () => {
  ...
  try {
    for (let i = 0; i < 1000; i++) {
      ...
      const text = formatTextWithNewLines(
        `This is the body of question number ${
          i + 1
        }. It contains detailed information about the question being asked. Let's make it long enough to require splitting into multiple lines as per the rules. ` +
          "X".repeat(500),
        30
      );
      ...
      const questionTags = tags
        .slice(randomInt(1, 5))
        .slice(0, randomInt(1, 5)); // Randomly select 1 to 5 tags from the available tags
      ...
      if (i < 990) {
        for (let j = 0; j < 50; j++) {
          const answerText = formatTextWithNewLines(
            `This is answer ${j + 1} for question ${
              i + 1
            }. It provides a detailed response to the question posed. ` +
              "Y".repeat(500),
            30
          );
          let answerDate;
          ...
          answers.push(
            await answerCreate(
              answerText,
              usernames[randomInt(0, usernames.length - 1)],
              answerDate
            )
          );
        }
    ...
    console.log(
      "Database populated with 1000 questions and their respective answers."
    );
  } catch (err) {
