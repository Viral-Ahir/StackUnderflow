...
describe("GET /getQuestionById/:qid", () => {
  ...
  it("should return a question object in the response when the question id is passed as request parameter", async () => {
    ...
    const mockPopulatedQuestion = {
      ...
      ask_date_time: mockQuestions
        .find((q) => q._id == mockReqParams.qid)!
        .ask_date_time.toISOString(),
    };

    // Provide mock question data
    (fetchAndIncrementQuestionViewsById as jest.Mock).mockResolvedValueOnce(
  ...
  it("should handle internal server error gracefully", async () => {
    ...
    // Mock the fetchAndIncrementQuestionViewsById to throw an error
    (fetchAndIncrementQuestionViewsById as jest.Mock).mockImplementation(() => {
