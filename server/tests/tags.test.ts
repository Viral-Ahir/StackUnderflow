...
describe("GET /getTagsWithQuestionNumber", () => {
  ...
  it("should return error 500 if getTagCountMap throws an error", async () => {
    (getTagCountMap as jest.Mock).mockRejectedValueOnce(
      new Error("Error fetching tags")
    );
  ...
  it("should return error 500 if tagCountMap contains an error", async () => {
    const mockErrorWrapped = { error: "Mocked error message" };
    (getTagCountMap as jest.Mock).mockResolvedValueOnce(mockErrorWrapped);
